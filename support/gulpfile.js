...
gulp.task('build-jcl', 'Build using JCL', function (callback) {
  var zosmf = (typeof process.env.ZOSMF === "undefined") ? "" : process.env.ZOSMF,
      command = 'zowe zos-jobs submit data-set "CUST001.BRIGHT.MARBLES.JCL(MBBLSP01)" --vasc' + zosmf;
  ...
  cmd.get(command, function (err, data, stderr) {
    if(err){
      ...
      var pattern = new RegExp(".*BIND.FOR.PLAN.MBDEMOSP.SUCCESSFUL.*","g");
      var found = data.match(pattern);
      if (!found) {
        callback(new Error("Error compiling"));
      } else {
        callback();
      }
  }});
...
gulp.task('verify-jcl', 'Verify JCL', function (callback) {
  var db2 = (typeof process.env.DB2 === "undefined") ? "" : process.env.DB2,
      command = 'zowe db2 call sp "EVENT.MBSTOR01(\'WHITE\',?,?)" --parameters 0 0 ' + db2;
  ...
  cmd.get(command, function (err, data, stderr) {
    if(err){
      ...
      var pattern = new RegExp(".*WHITE.*","g");
      var found = data.match(pattern);
      if (!found) {
        callback(new Error("Not Validated"));
      } else {
        callback();
      }
  }});
...
gulp.task('verify-cobol', 'Verify COBOL', function (callback) {
  var zosmf = (typeof process.env.ZOSMF === "undefined") ? "" : process.env.ZOSMF,
      command = 'zowe zos-jobs submit data-set "CUST001.BRIGHT.MARBLES.JCL(MBBLMN01)" --vasc ' + zosmf;
  ...
  cmd.get(command, function (err, data, stderr) {
    if(err){
      ...
      var pattern = new RegExp(".*WHITE.*[0-9][0-9][0-9][0-9].*[0-9][0-9][0-9][0-9].*","g");
      var found = data.match(pattern);
      if (!found) {
        callback(new Error("Not Validated"));
      } else {
...
gulp.task('verify', 'Verify Cobol and JCL Calls', gulpSequence('verify-cobol', 'verify-jcl'));
